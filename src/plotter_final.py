...
RESULTS_SETS = {
    ...
    'final-final-results': {
        'results_directory': 'final-final-results',
        'cv_results_':       '2024-06-10-022518__GridSearchCV.pkl',
    }
}

# %% Configuration

FEATURE_SELECTION_METRIC = 'final-final-results'  # 'accuracy', 'roc_auc', 'f1'
...
SVC_DEFAULT_PARAMS = {
    'param_C':   1.0, 'param_degree': 3, 'param_coef0': 0.0, 'param_gamma': 'scale', 'param_class_weight': 'None',
    'param_tol': 0.001
}
SVC_DEFAULT_KERNEL = 'rbf'
GAMMA_PARAMS = ['scale', 0.00001, 0.0001, 0.001, 0.01, 0.1, 1.0, 10.0, 100.0]

# GAMMA_PARAMS = ['scale', 0.00001, 0.0001, 0.001, 0.01, 0.1, 1.0, 10.0, 100.0]
# for g in GAMMA_PARAMS:
#     gamma_mask1 = cv_results['param_gamma'] == g
#     cv_results_gamma = cv_results.copy()[gamma_mask1]
#
#     all_best_params_gamma = get_best_parameter_set_for_kernel(
#         kernel=False, cv_results_=cv_results_gamma, gridsearch_metric=GRIDSEARCH_METRIC,
#         params_to_get_best=PARAMS_TO_GET_BEST, alpha_=alpha, beta_=beta, svc_default_params=SVC_DEFAULT_PARAMS
#     )
#     # Get metrics for the best parameters
#     best_row_gamma = utils.get_row_from_best_params(cv_results_gamma, all_best_params_gamma, CHOSEN_TOL, CHOSEN_CLASS_WEIGHT, GRIDSEARCH_METRIC)
#
#     print(f"\n\nBest parameters for gamma={g}:")
#     # Print the best parameters dict nicely formatted
#     for key, value in all_best_params_gamma.items():
#         print(f"{key}={utils.format_number(value, latex=True)}")
#
#     # Print metrics for the best parameters
#     for metric in ['roc_auc', 'accuracy', 'f1', 'recall', 'precision']:
#         print(f"{metric}: {utils.round_to_n(best_row_gamma[f'mean_test_{metric}'].values[0], 3)} ± {utils.round_to_n(best_row_gamma[f'std_test_{metric}'].values[0], 3)}")
...
# print(
#     f"Number of unique values in gamma_float (should be 5 if 'scale'='auto'): {cv_results['gamma_float'].nunique()}"
#     )
#
...
def plot_parameter_effects_stacked(cv_results_, parameters, metrics, fixing_mode='optimal', default_params=None,
                                   params_in_title=True, fixed_gamma=None):
    """
    ...
    :param params_in_title: (bool) Whether to include the fixed parameters in the title.
    :param fixed_gamma: (int) The fixed gamma value to use for the plots.
    """
    ...
    if fixing_mode not in ['default', 'optimal', 'both'] or fixed_gamma is None:
        raise ValueError("Fixing_mode must be 'default', 'optimal', or 'both'! Gamma must be set!")
    ...
    for param in parameters:
        ...
        #     continue
        fixed_params_ = [p for p in parameters if (p != param) and (p != 'param_gamma')]
        ...
        best_params_ = None
        for idx, (metric_, mode) in enumerate([(metric_, mode) for metric_ in metrics for mode in fixing_modes]):
            best_params_ = utils.get_best_params(
                cv_results_, GRIDSEARCH_METRIC, parameters, alpha=alpha, beta=beta
            ) if mode == 'optimal' else pd.Series(default_params).reindex(parameters)

            mask = pd.DataFrame(
                (cv_results_[fixed_params_] == best_params_.drop(index=param))
            ).all(axis=1)
            varying_param_data = deepcopy(cv_results_[mask])

            x = varying_param_data[param]
            y = varying_param_data[f'mean_test_{metric_}']
            std = varying_param_data[f'std_test_{metric_}']
            ...
            if mode == 'default':
                ...
                marker = 'D'
                linestyle = '-'
            if last_used_color is not None and fixing_mode == 'both' and mode == 'optimal' and len(metrics) * len(
                    fixing_modes
                    ) > 2:
                ...
                #         label=f'{metric_} ({mode})'
                #     )
                #     ax.set_xticks(x)
                #     ax.set_xticklabels(x, rotation=0)
                # else:
                p = ax.plot(
                    x, y, linestyle=linestyle, marker=marker, color=color, label=f'{metric_} ({mode})'
                )
                ax.fill_between(
                    x, y - se, y + se, alpha=0.2,
                    label=f'{f'SE ({metric_}, {mode})' if len(metrics) * len(fixing_modes) < 3 else ''}'
                )
            else:
                # if param == 'param_degree':
                #     p = ax.errorbar(
                ...
                # else:
                p = ax.plot(
                    x, y, linestyle=linestyle, marker=marker, label=f'{metric_} ({mode})'
                )
                ax.fill_between(
                    x, y - se, y + se, alpha=0.2,
                    label=f'{f'SE ({metric_}, {mode})' if len(metrics) * len(fixing_modes) < 1 else ''}'
                )
            try:
        ...
        test_metric_label = ', '.join([
            metric_.replace('_', ' ').upper() if metric_ != 'accuracy' else 'Accuracy'
            for metric_ in metrics
        ])
        fixed_opt_param_label = ', '.join([
            f'{prm.split("_")[1]}={utils.format_number(best_params_[prm] if prm != 'param_poly' else int(best_params_[prm]), latex=True)}'
            for prm in fixed_params_ if prm != 'param_gamma'
        ])
        defaults_param_label = ', '.join(
            [
                f'{prm.split("_")[1]}={utils.format_number(default_params[prm] if prm != 'param_poly' else int(default_params[prm]), latex=True)}'
                for prm in fixed_params_ if prm != 'param_gamma']
            )
        if params_in_title:
            title = (
                f'Effect of {param.split("_")[1]} on Model Performance ({test_metric_label}) With gamma={fixed_gamma} \n'
                f' {f"for Default ({defaults_param_label}) and Optimal Parameters ({fixed_opt_param_label})" if fixing_mode == "both" 
                    else f"Using Default Parameters ({defaults_param_label})" if mode == "default" 
                    else f"Using Optimal Parameters ({fixed_opt_param_label})"}'
            )
        else:
            title = (
                f'Effect of {param.split("_")[1]} on Model Performance ({test_metric_label})'
                f' {f"for Default and Optimal Parameters" if fixing_mode == "both" 
                    else f"Using Default Parameters" if mode == "default" 
                    else "Using Optimal Parameters"}'
            )

        ax.set_xlabel(param_label)
        ax.set_ylabel(f'Mean CV {test_metric_label}')
        ax.set_title(title)
        ...
        plt.show()
        for c in ['$', '}', '{']:
            fixed_opt_param_label = fixed_opt_param_label.replace(c, '')
            defaults_param_label = defaults_param_label.replace(c, '')
        fixed_opt_param_text = fixed_opt_param_label.replace(', ', '-')
        defaults_param_text = defaults_param_label.replace(', ', '-')
        figname = f'gamma={fixed_gamma}-stacked-effect-param-{param_label}_{test_metric_label.replace(', ', '-').replace(' ', '-')}_{fixing_mode}_fixopt-{fixed_opt_param_text}_fixdef-{defaults_param_text}.png'.replace(', ', '-')
        fig.savefig(
            CV_RESULTS_DIR / figname,
        ...
        plt.pause(1)
        time.sleep(0.1)
        # print(f"Saved heatmap for {param_label} with fixed {fixed_params_text} to file.")
...
def plot_parameter_effects_stacked_bars(cv_results_, parameters, metrics, fixing_mode='optimal', default_params=None,
    ...
    base_colors = plt.get_cmap('tab10', len(metrics))
    custom_colors = {
        'f1':        '#5D3FD3',  # Purple
    ...
    for idx, (metric, mode) in enumerate([(metric, mode) for metric in metrics for mode in fixing_modes]):
        best_params_ = utils.get_best_params(cv_results_, GRIDSEARCH_METRIC, parameters, alpha=alpha, beta=beta) \
            if mode == 'optimal' else pd.Series(default_params).reindex(parameters)
    ...
    defauls_param_label = ', '.join(
        [
            f'{prm.split("_")[1]}={utils.format_number(default_params[prm] if prm != "param_degree" else int(default_params[prm]), latex=True)}'
            for prm in fixed_params_]
    )
    if params_in_title:
        title = (
            f'Effect of {param.split("_")[1]} on Model Performance ({test_metric_label})\n'
            f' {f"for Default ({defauls_param_label}) and Optimal Parameters ({fixed_opt_param_label})" if fixing_mode == "both" else f"Using Default Parameters ({defauls_param_label})" if mode == "default" else f"Using Optimal Parameters ({fixed_opt_param_label})"}'
        )
    else:
        title = (
            f'Effect of {param.split("_")[1]} on Model Performance ({test_metric_label})'
            f' {"for Default and Optimal" if fixing_mode == "both" else "Using Default" if mode == "default" else "Using Optimal"} Parameters'
        )
    ...
    fig.savefig(
        CV_RESULTS_DIR / f'gamma={gamma_param}-stacked-effect-param-{param_label}_{test_metric_label.replace(", ", "-").replace(" ", "-")}_{fixing_mode}_fixed-{fixed_params_text}.png',
        dpi=300, transparent=True, bbox_inches='tight'
    )
    plt.pause(0.5)
    plt.close(fig)
...
def plot_parameter_effects_opt(cv_results_, parameters, test_metric=None, default_params=None):
    ...
    if default_params is None:
        ...
        best_param_vals = pd.Series(
            {
                'param_C':     best_row['param_C'], 'param_degree': best_row['param_degree'],
                'param_coef0': best_row['param_coef0']
            }
            )
    else:
    ...
    for fixed_kernel_ in cv_results_['param_kernel'].unique():
        ...
        for fixed_normalizer in cv_results_['param_normalizer'].unique():
            ...
            for param in parameters:
                ...
                std = varying_param_data[
                    f'std_test_{test_metric}']  # see: https://github.com/scikit-learn/scikit-learn/discussions/20680
                ...
                title = (
                    ...
                    f'using {f"Default" if default_params else "Optimal"} Parameters'
                )
                ...
                if param == 'param_degree':
                    ...
                    if default_params:
                        ...
                        arrow_pos = (
                        max_accuracy_value, max_accuracy_score + max_accuracy_std + 0.02 * max_accuracy_std)
                        arrow_length = 0.0045
                    else:
                ...
                if default_params:
                    fig.savefig(
                        CV_RESULTS_DIR / f'effect-param-{param_label}_fixed-{fixed_kernel_}__fixed-{fixed_kernel_}_{metrics_label}-defaults.png',
                        dpi=300, transparent=True, bbox_inches='tight'
                        )
                else:
                    fig.savefig(
                        CV_RESULTS_DIR / f'effect-param-{param_label}_fixed-{fixed_normalizer}_fixed-{fixed_kernel_}_{metrics_label}-opt.png',
                        dpi=300, transparent=True, bbox_inches='tight'
                        )
                ...
                plt.pause(1)
                time.sleep(0.1)
                # print(f"Saved heatmap for {param_label} with fixed {fixed_kernel_} and {fixed_normalizer} to file.")


# TODo: add_label_band flyttad till utils

def create_heatmap(cv_results_, x_param, y_param, fixed_params_, test_metric, default_params=None,
                   use_default_params=False, params_in_title=True):
    ...
    aggregated_results = filtered_results.drop_duplicates(subset=[x_param, y_param])  # TODO: reverse order on x-data
    ...
    if y_param == 'param_coef0':
        custom_order = sorted(filtered_results[y_param].unique(), reverse=True)
    elif y_param == 'param_degree':
    ...
    if x_param == 'param_deg':
        ...
        fig, ax = plt.subplots(figsize=(min(12, 2 * heatmap_data.shape[1]), min(10, 2 * heatmap_data.shape[0])))
    ...
    sns.heatmap(
        ...
        annot_kws={"size": 10},
        cbar_kws={'label': f'Mean CV {test_metric_label} {'' if labels is None else '(±SE)'}', 'shrink': shrink_val},
    ...
    if 'param_gamma' not in fixed_params_:
        fixed_param_label = fixed_param_label
        fixed_param_text = fixed_param_label

    x_param_label = x_param.replace('param_', '')
    y_param_label = y_param.replace('param_', '')
    title = (
        f'Interaction of {x_param_label} and {y_param_label} for gamma={gamma_param}\n'
        f'using {f"Default" if use_default_params else "Optimal"} Parameters'
    )
    if params_in_title:
        title += f' ({test_metric_label})'
    ...
    figname = f"gamma={gamma_param}-heatmap-{x_param_label}-{y_param_label}_fixed-{'defaults' if use_default_params else 'opt'}-{fixed_param_text}.png"
    fig.savefig(
        CV_RESULTS_DIR / figname,
        dpi=300, transparent=True, bbox_inches='tight'
    )
    ...
    # print(f"Saved heatmap for {x_param} vs {y_param} with fixed params {fixed_params_}")


def get_best_parameter_set_for_kernel(kernel, cv_results_, gridsearch_metric, params_to_get_best, alpha_, beta_,
                                      svc_default_params):
    # TODO: lägga till gamma som parameter
    if kernel:
        best_params_ = utils.get_best_params(
            cv_results_[cv_results_['param_kernel'] == kernel], gridsearch_metric, params_to_get_best, alpha=alpha_,
            beta=beta_, default_params=svc_default_params
        )
    else:
        best_params_ = utils.get_best_params(
            cv_results_, gridsearch_metric, params_to_get_best, alpha=alpha_, beta=beta_,
            default_params=svc_default_params
            )
...
print_all_best_params_global = True
if print_all_best_params_global:
    all_best_params_gamma_global = get_best_parameter_set_for_kernel(
        kernel=False, cv_results_=cv_results, gridsearch_metric=GRIDSEARCH_METRIC,
        params_to_get_best=PARAMS_TO_GET_BEST, alpha_=alpha, beta_=beta, svc_default_params=SVC_DEFAULT_PARAMS
        )

    # Get metrics for the best parameters
    best_row_gamma = utils.get_row_from_best_params(
        cv_results, all_best_params_gamma_global, CHOSEN_TOL, CHOSEN_CLASS_WEIGHT, GRIDSEARCH_METRIC
        )

    print(
        f"\n# =========================================== #"
        f"\n# |       RESULTS FOR METRIC: {GRIDSEARCH_METRIC.upper()}" + " " * (14 - len(GRIDSEARCH_METRIC)) + "| #"
        f"\n# =========================================== #"
        )
    print(
        f"\n# ------------------------------------------- #"
        f"\n# |  BEST PARAMETERS GLOBAL (ALL GAMMA)     | #"
        f"\n# ------------------------------------------- #"
        )
    # Print the best parameters dict nicely formatted
    print("\nBEST PARAMETERS:")
    for key, value in all_best_params_gamma_global.items():
        print(f"{key}={utils.format_number(value, latex=True)}")

    # Print metrics for the best parameters
    print("\nMETRICS:")
    for metric in ['f1', 'roc_auc', 'recall', 'precision', 'accuracy']:
        print(
            f"{metric}: {utils.round_to_n(best_row_gamma[f'mean_test_{metric}'].values[0], 6)} ± {utils.round_to_n(best_row_gamma[f'std_test_{metric}'].values[0], 6)}"
            )

    # Print top 5 rows from cv_results_gamma sorted according to metric (parameters and scores) without abbreviating
    print("\nTOP 5 ROWS:")
    top5 = cv_results.copy().sort_values(
        by=f'mean_test_{GRIDSEARCH_METRIC}', ascending=False
    ).loc[:, [
        'param_C', 'param_degree', 'param_coef0', 'param_tol', 'param_kernel', f'mean_test_roc_auc',
        f'std_test_roc_auc', f'mean_test_accuracy', f'std_test_accuracy', f'mean_test_f1', f'std_test_f1',
        f'mean_test_recall', f'std_test_recall', f'mean_test_precision', f'std_test_precision'
    ]].head(5)
    columns_names_to_format = 'param_C', 'param_degree', 'param_coef0', 'param_tol', 'param_kernel'
    columns_to_format = top5.loc[:, columns_names_to_format].copy()
    for col in columns_names_to_format:
        top5[col] = top5[col].astype(str)
    top5.loc[:, columns_names_to_format] = columns_to_format.map(
        lambda x: utils.format_number(x, latex=True)
    )
...
for gamma_param in GAMMA_PARAMS:
    ...
    parameters_in_title = True
    print_all_best_params = True
    if print_all_best_params:
        # Get metrics for the best parameters
        best_row_gamma = utils.get_row_from_best_params(
            cv_results_gamma, all_best_params_gamma, CHOSEN_TOL, CHOSEN_CLASS_WEIGHT, GRIDSEARCH_METRIC
            )

        print(
            f"\n# ------------------------------------------- #"
            f"\n# |  BEST PARAMETERS FOR GAMMA={gamma_param}" + " " * (13 - len(str(gamma_param))) + "| #"
                                                                                                     f"\n# ------------------------------------------- #"
            )
        # Print the best parameters dict nicely formatted
        print("\nBEST PARAMETERS:")
        ...
        # Print top 5 rows from cv_results_gamma sorted according to metric (parameters and scores) without abbreviating
        print("\nTOP 5 ROWS:")
        top5 = cv_results_gamma.sort_values(
            by=f'mean_test_{GRIDSEARCH_METRIC}', ascending=False
        ).loc[:, [
                     'param_C', 'param_degree', 'param_coef0', 'param_tol', 'param_kernel', f'mean_test_roc_auc',
                     f'std_test_roc_auc', f'mean_test_accuracy', f'std_test_accuracy', f'mean_test_f1', f'std_test_f1',
    ...
    barplots = False
    if barplots:
        plot_parameter_effects_stacked_bars(
            cv_results_=cv_results_gamma, parameters=PARAMS_OF_INTEREST, metrics=['roc_auc'], fixing_mode='both',
            default_params=SVC_DEFAULT_PARAMS, params_in_title=parameters_in_title
            )
        plot_parameter_effects_stacked_bars(
            cv_results_=cv_results_gamma, parameters=PARAMS_OF_INTEREST, metrics=['accuracy'], fixing_mode='both',
            default_params=SVC_DEFAULT_PARAMS, params_in_title=parameters_in_title
            )
        plot_parameter_effects_stacked_bars(
            cv_results_=cv_results_gamma, parameters=PARAMS_OF_INTEREST, metrics=['f1', 'recall', 'precision'],
            fixing_mode='both', default_params=SVC_DEFAULT_PARAMS, params_in_title=parameters_in_title
            )
    ...
    stacked_lines = True
    if stacked_lines:
        plot_parameter_effects_stacked(
            cv_results_=cv_results_gamma, parameters=PARAMS_OF_INTEREST, metrics=['roc_auc'], fixing_mode='both',
            default_params=SVC_DEFAULT_PARAMS, params_in_title=parameters_in_title, fixed_gamma=gamma_param
        )
        ...
        plot_parameter_effects_stacked(
            cv_results_=cv_results_gamma, parameters=PARAMS_OF_INTEREST, metrics=['f1', 'recall', 'precision'],
            fixing_mode='both', default_params=SVC_DEFAULT_PARAMS, params_in_title=parameters_in_title, fixed_gamma=gamma_param
            )
    ...
    if param_effects:
        ...
        plot_parameter_effects_opt(
            cv_results_gamma, PARAMS_OF_INTEREST, test_metric=GRIDSEARCH_METRIC, default_params=None
            )  # SVC_DEFAULT_PARAMS)
        plot_parameter_effects_opt(
            cv_results_gamma, PARAMS_OF_INTEREST, test_metric=GRIDSEARCH_METRIC, default_params=SVC_DEFAULT_PARAMS
            )
    ...
    for default in [True, False]:
        ...
        if default:
            pass  # continue  # TODO: REMOVE
        else:
            print(f"Best degree to plot: {all_best_params_gamma['param_degree']}")
        ...
        if not default:
            ...
            create_heatmap(
                deepcopy(cv_results_gamma[cv_results_gamma['param_kernel'] != 'rbf']), 'param_C', 'param_coef0',
                fixed_parameters, PLOT_METRIC, default_params=SVC_DEFAULT_PARAMS, use_default_params=default,
                params_in_title=parameters_in_title
            )
        ...
        create_heatmap(
            deepcopy(cv_results_gamma[cv_results_gamma['param_kernel'] == 'poly']), 'param_C', 'param_degree',
            fixed_parameters, PLOT_METRIC, default_params=SVC_DEFAULT_PARAMS, use_default_params=default,
            params_in_title=parameters_in_title
        )
        ...
        create_heatmap(
            deepcopy(cv_results_gamma[cv_results_gamma['param_kernel'] == 'poly']), 'param_coef0', 'param_degree',
            fixed_parameters, PLOT_METRIC, default_params=SVC_DEFAULT_PARAMS, use_default_params=default,
            params_in_title=parameters_in_title
        )
